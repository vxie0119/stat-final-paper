...
**Correlation between surface and injury occurrence**
...
# We need to convert 'FieldType' into a binary variable for logistic regression
# Let's assume 'Synthetic' as 1 and 'Natural' as 0
merged_data['FieldTypeBinary'] = (merged_data['FieldType'] == 'Synthetic').astype(int)

# Dropping rows where 'FieldType' is NaN since we can't use them in the regression
merged_data_clean = merged_data.dropna(subset=['FieldType'])

# Logistic Regression: InjuryOccurred ~ FieldType
X = merged_data_clean[['FieldTypeBinary']]  # Independent variable
X = sm.add_constant(X)  # Adding a constant for the intercept term
y = merged_data_clean['InjuryOccurred']  # Dependent binary variable
...
No significant association between weather conditions and types of injuries.

**Does temperature affect injury occurrence?**
...
*Predicts whether or not an injury will occur based on temperature*

Whether the results are hot or cold, both have a significant impact on the chance of injuries. 
...
+ https://pubmed.ncbi.nlm.nih.gov/30697503/
